...
const calculateRange = (date, localizer, biweek) => {
  if(biweek) {
    
    let first = dates.startOf(date, 'week', localizer.startOfWeek())

    let start = first
    let end = dates.add(start, 13, 'day')
    let current = start
    let range = []
    ...
    return dates.visibleDays(date, localizer)
  }
...
function (_React$Component) {
  ...
  _proto.render = function render() {
    ...
    var _this$props4 = this.props,
        ...
        biweek = _this$props4.biweek,
        className = _this$props4.className,
        month = calculateRange(date, localizer, biweek),
